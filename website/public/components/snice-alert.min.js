        ...
        <div class="alert-region" role="alert" aria-live="polite">
          <if value="${e}">
            <div class="${this.icon&&"none"!==this.icon?"alert-icon":"alert-icon alert-icon--default"}" part="icon">
              <slot name="icon">
                ${this.icon?function(e,r="icon"){if(!e)return t.html``;const{kind:i,value:n}=function(e){if(e.startsWith("img://"))return{kind:"img",value:e.slice(6)};if(e.startsWith("text://"))return{kind:"text",value:e.slice(7)};const t=/^(https?:\/\/|\/|\.\/|\.\.\/|data:)/.test(e),r=/^[^:]*\w\.(svg|png|jpe?g|jfif|pjp|gif|webp|avif|jxl|ico|cur|bmp|tiff?|heic|heif|apng)(\?.*)?$/i.test(e);return t||r?{kind:"img",value:e}:c[e]?{kind:"registry",value:e}:{kind:"text",value:e}}(e);return"img"===i?t.html`<img class="${r}" src="${n}" alt="" part="icon" />`:"registry"===i?t.html`<span class="${r}" part="icon">${t.unsafeHTML(c[n])}</span>`:/^[a-z][a-z0-9_]*$/.test(n)?t.html`<span class="${r} snice-icon-ligature" style="font-family:var(--snice-icon-font,'Material Symbols Outlined'),sans-serif" part="icon">${n}</span>`:t.html`<span class="${r}" part="icon">${n}</span>`}(this.icon,"alert-icon-content"):this.defaultIconSvg()}
              </slot>
            </div>
          </if>
