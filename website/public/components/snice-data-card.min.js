      ...
      <div class="data-card" part="container">
        <div class="data-card__header" part="header">
          ...
          <button
            ...
            style="display: ${this.fields.some(e=>!1!==e.editable)?"":"none"}">
            <span class="data-card__edit-icon" aria-hidden="true">${r.unsafeHTML(this.editable?a:n)}</span>
            ${this.editable?"Done":"Edit"}
          </button>
        </div>

        ${e.map(e=>this.renderGroup(e))}
      </div>
    `}renderGroup(e){return r.html`
      <div class="data-card__group" part="group">
        <if ${e.name}>
          <div class="data-card__group-title" part="group-title">${e.name}</div>
        </if>
        ${e.fields.map(e=>this.renderField(e))}
      </div>
    `}renderField(e){const i=this.editingField===e.label,t=this.editable&&!1!==e.editable;return r.html`
      <div class="field" part="field">
        <if ${e.icon}>
          <span class="field__icon" part="field-icon">${function(e,i="icon"){if(!e)return r.html``;const{kind:t,value:a}=function(e){if(e.startsWith("img://"))return{kind:"img",value:e.slice(6)};if(e.startsWith("text://"))return{kind:"text",value:e.slice(7)};const r=/^(https?:\/\/|\/|\.\/|\.\.\/|data:)/.test(e),i=/^[^:]*\w\.(svg|png|jpe?g|jfif|pjp|gif|webp|avif|jxl|ico|cur|bmp|tiff?|heic|heif|apng)(\?.*)?$/i.test(e);return r||i?{kind:"img",value:e}:l[e]?{kind:"registry",value:e}:{kind:"text",value:e}}(e);return"img"===t?r.html`<img class="${i}" src="${a}" alt="" part="icon" />`:"registry"===t?r.html`<span class="${i}" part="icon">${r.unsafeHTML(l[a])}</span>`:/^[a-z][a-z0-9_]*$/.test(a)?r.html`<span class="${i} snice-icon-ligature" style="font-family:var(--snice-icon-font,'Material Symbols Outlined'),sans-serif" part="icon">${a}</span>`:r.html`<span class="${i}" part="icon">${a}</span>`}(e.icon,"field__icon-glyph")}</span>
        </if>
        <span class="field__label" part="field-label">${e.label}</span>
        <if ${i}>
          <input
            class="field__input"
            part="field-input"
          ...
          />
          <button class="field__save-btn" part="field-save" @click=${()=>this.handleSave(e)}>Save</button>
        </if>
        <if ${!i}>
          ${this.renderValue(e,t)}
        </if>
      </div>
    `}renderValue(e,i){const t=e.type||"text";return"link"===t?r.html`
        <a class="field__value field__value--link" part="field-value" href="${e.href||"#"}" target="_blank" rel="noopener">${e.value}</a>
      `:"badge"===t?r.html`
        <span class="field__value field__value--badge field__value--badge-${e.badgeVariant||"default"}" part="field-value">${e.value}</span>
        <if ${i}>
          <button class="field__save-btn" part="field-edit" @click=${()=>this.startEdit(e)} style="background:transparent;color:var(--snice-color-text-tertiary,rgb(115 115 115));border:1px solid var(--snice-color-border,rgb(226 226 226))">✎</button>
        </if>
      `:"date"===t?r.html`
        <span class="field__value field__value--date" part="field-value" @click=${()=>i&&this.startEdit(e)} style="${i?"cursor:pointer":""}">${e.value}</span>
        <if ${i}>
          <button class="field__save-btn" part="field-edit" @click=${()=>this.startEdit(e)} style="background:transparent;color:var(--snice-color-text-tertiary,rgb(115 115 115));border:1px solid var(--snice-color-border,rgb(226 226 226))">✎</button>
        </if>
      `:"currency"===t?r.html`
        <span class="field__value field__value--currency" part="field-value" @click=${()=>i&&this.startEdit(e)} style="${i?"cursor:pointer":""}">${e.value}</span>
        <if ${i}>
          <button class="field__save-btn" part="field-edit" @click=${()=>this.startEdit(e)} style="background:transparent;color:var(--snice-color-text-tertiary,rgb(115 115 115));border:1px solid var(--snice-color-border,rgb(226 226 226))">✎</button>
        </if>
      `:r.html`
      <span class="field__value" part="field-value" @click=${()=>i&&this.startEdit(e)} style="${i?"cursor:pointer":""}">${e.value}</span>
      <if ${i}>
        <button class="field__save-btn" part="field-edit" @click=${()=>this.startEdit(e)} style="background:transparent;color:var(--snice-color-text-tertiary,rgb(115 115 115));border:1px solid var(--snice-color-border,rgb(226 226 226))">✎</button>
      </if>
    `}},o})();return e.SniceDataCard=d,e}({},Snice);
//# sourceMappingURL=snice-data-card.min.js.map
