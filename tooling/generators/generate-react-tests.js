...
// Tests generated before the generator moved out of scripts/ carry this marker.
const LEGACY_GENERATED_MARKER = 'Auto-generated by scripts/generate-react-tests.js';

/**
 * Discover released component directories (WIP-excluded, sorted for determinism).
 */
export function discoverTestComponents(componentsDir = path.join(projectRoot, 'packages', 'components', 'src')) {
  const wip = getWipComponents();
  const components = [];

  for (const item of fs.readdirSync(componentsDir).sort()) {
    if (wip.has(item) || item === 'theme') continue;
    const componentDir = path.join(componentsDir, item);
    if (!fs.statSync(componentDir).isDirectory()) continue;
    if (fs.existsSync(path.join(componentDir, `snice-${item}.ts`))) {
...
/**
 * Delete auto-generated test files whose component is no longer released
 * (removed or moved to .wip). Hand-written tests lack the generated marker
 * and are never touched. Returns the deleted file names, sorted.
 */
export function removeStaleGeneratedTests(testsDir, validComponents) {
  const valid = new Set(validComponents);
  const removed = [];

  if (!fs.existsSync(testsDir)) return removed;

  for (const file of fs.readdirSync(testsDir).sort()) {
    if (!file.endsWith('.test.tsx')) continue;
    const componentName = file.slice(0, -'.test.tsx'.length);
    if (valid.has(componentName)) continue;

    const filePath = path.join(testsDir, file);
    const content = fs.readFileSync(filePath, 'utf-8');
    if (!content.includes(GENERATED_MARKER) && !content.includes(LEGACY_GENERATED_MARKER)) continue;
 ...
 */
export function generateTestFile(componentName, config = {}) {
...
function generateTests() {
  ...
  const components = discoverTestComponents(componentsDir);
  ...
  const removed = removeStaleGeneratedTests(testsDir, components);
  for (const file of removed) {
    console.log(`  Removed stale: ${file}`);
  }

  console.log(`\n✅ Test generation complete!\n`);
  console.log(`Generated ${generated} test files, removed ${removed.length} stale`);
  console.log(`\nRun tests with: npm test tests/react-adapters\n`);
}

const invokedPath = process.argv[1] ? path.resolve(process.argv[1]) : '';
if (invokedPath === fileURLToPath(import.meta.url)) generateTests();
