#!/usr/bin/env node
// Assembles website/showcases/*/card.html into website/public/components.html.
// A flat website/public/showcases/ compatibility tree is emitted as part of the
// website build so the deployed artifact remains byte-for-byte compatible with
// the former public/showcases/ layout.
...
const dir = join(root, 'website', 'showcases');
const sharedDir = join(dir, 'shared');
const publicDir = process.env.SNICE_WEBSITE_PUBLIC_DIR || join(root, 'website', 'public');
const legacyDir = join(publicDir, 'showcases');
const manifest = JSON.parse(readFileSync(join(sharedDir, 'manifest.json'), 'utf-8'));
const wip = getWipComponents();

function fragmentPath(file) {
  if (file.startsWith('_')) return join(sharedDir, file);
  return join(dir, file.replace(/\.html$/, ''), 'card.html');
}

// Preserve every legacy flat fragment, including cards that are intentionally
// not in the assembly manifest. These files are part of the deployed website.
rmSync(legacyDir, { recursive: true, force: true });
mkdirSync(legacyDir, { recursive: true });
for (const file of readdirSync(sharedDir)) {
  if (file.endsWith('.html') || file === 'manifest.json') {
    cpSync(join(sharedDir, file), join(legacyDir, file));
  }
}
for (const entry of readdirSync(dir).sort()) {
  if (entry === 'shared') continue;
  const card = join(dir, entry, 'card.html');
  if (existsSync(card)) cpSync(card, join(legacyDir, `${entry}.html`));
}
...
for (const file of middle) {
  if (file.startsWith('_cat-')) {
    ...
    const content = readFileSync(fragmentPath(file), 'utf-8');
...
for (const cat of categories) {
  const content = readFileSync(fragmentPath(cat.file), 'utf-8');
...
for (const comp of components) {
  let content = readFileSync(fragmentPath(comp.file), 'utf-8');
...
const sidebarLinks = [];
for (const comp of components) {
  const content = readFileSync(fragmentPath(comp.file), 'utf-8');
