 ...
 */
export function extractComponentMetadata(tagName: string): {
  properties: string[];
 ...
 */
export function isFormAssociated(tagName: string): boolean {
  const constructor = customElements.get(tagName);
  if (!constructor) return false;

  return 'formAssociated' in constructor && (constructor as any).formAssociated === true;
}
 ...
 */
export async function waitForComponentDefinition(
  tagName: string,
  timeout = 5000
): Promise<void> {
