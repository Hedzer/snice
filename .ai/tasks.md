...
### Task 117: Bin-Pack Layout (`snice-bin-pack`)
A JS-driven bin-packing layout that arranges variable-sized children to minimize wasted space. Unlike `snice-masonry` (CSS columns, top-down flow), this uses a real bin-pack algorithm to find optimal placement — items go wherever they fit best, not just sequentially into columns. Properties: `gap`, `direction` (row/column priority). Should observe child resize/add/remove and re-layout automatically. Works with any child elements of any size.
