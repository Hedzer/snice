...
const storyUrl = 'http://localhost:6006/iframe.html?id=datetimepicker--form-integration&viewMode=story';

test('Storybook date-time form story is native, interactive, responsive, and theme-complete', async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    const pickers = Array.from(document.querySelectorAll('snice-date-time-picker'));
  ...
  const form = page.locator('#date-time-picker-story-form');
  const appointment = page.locator('#date-time-picker-story-appointment');
  const legend = page.locator('#date-time-picker-story-legend');
  const fieldset = page.locator('#date-time-picker-story-fieldset');
  ...
  expect(await appointment.evaluate((picker: any) => ({
    ...
    display: '10/03/2026 14:05:00'
  });

  await page.goto(`${storyUrl}&globals=theme:light`, { waitUntil: 'domcontentloaded' });
  await page.waitForFunction(() => document.documentElement.getAttribute('data-theme') === 'light');
  await expect(page.locator('#date-time-picker-story-appointment')).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth))
    .toBeLessThanOrEqual(await page.evaluate(() => document.documentElement.clientWidth));
