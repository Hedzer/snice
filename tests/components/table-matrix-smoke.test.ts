...
//
// `tests/components/table-matrix` (52 files, ~100s) is excluded from the default
...
// pay, and it deliberately lives OUTSIDE that directory so it stays collected.
//
...
import {
  ...
  type MatrixColumn,
} from './table-matrix/matrix-utils';
import { expectEmptyState, expectLoadingState } from './table-matrix/delivery-support';
import { stubLayout, fillRow, TALL } from './table-matrix/height-fill-support';
import { displayText } from './table-matrix/typed-cells-support';
import { expectVirtualWindow, expectWindowCells } from './table-matrix/virtualization-support';
...
/** The slice families of tests/components/table-matrix, in file-prefix order. */
const FAMILIES = [
